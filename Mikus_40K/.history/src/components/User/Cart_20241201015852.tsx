...
const Cart: React.FC = () => {
  ...
  const cartItems = useSelector((state: any) => state.cart.items); // Obtener el carrito del estado de Redux
        ...
        <tbody>
          {cartProducts.map((item) => (
            <tr key={item.id}>
              ...
              <td>
                {cartItems.find((cartItem) => cartItem.productId === item.id)?.quantity || 0}
              </td>
              <td>
